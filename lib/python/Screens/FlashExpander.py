...
class FlashExpander(Setup):
	...
	def showDeviceSelection(self):
		def readDevicesCallback():
			...
			self.session.openWithCallback(self.keySelectCallback, MessageBox, text=message, list=choiceList)
	...
	def keyGreen(self):
		def activateCallback(output, retVal, extraArgs):
			def mountCallback(output, retVal, extraArgs):
				...
				if isdir(join(f"/{EXPANDER_MOUNT}/{EXPANDER_MOUNT}", "bin")):
					...
					self.session.open(MessageBox, _("Error: mount not successful!"), MessageBox.TYPE_ERROR)

			self.messageBox.close(retVal == 0)
			if retVal:
				self.session.open(MessageBox, _("Error: The copy was unsuccessful!"), MessageBox.TYPE_ERROR)
				self.updateStatus()
			else:
		...
		def copyFlashdata():
			...
			self.messageBox = self.session.open(MessageBox, "%s\n\n%s" % (usrDirectory, _("Please wait, flash memory is being copied to this flash expander. This could take a number of minutes to complete.")), MessageBox.TYPE_INFO, enable_input=False)

		def deleteCallback(answer):
			def deleteMessageCallback(output, retVal, extraArgs):
				self.messageBox.close(retVal == 0)
				if retVal:
					self.session.open(MessageBox, _("Error: The delete was unsuccessful!"), MessageBox.TYPE_ERROR)
				self.updateStatus()

			if answer:
				self.console.ePopen(["/bin/busybox", "rm", "-r", usrDirectory], callback=deleteMessageCallback)
				self.messageBox = self.session.open(MessageBox, "%s\n\n%s" % (usrDirectory, _("Please wait, copy of flash memory is being deleted. This could take a few minutes to complete.")), MessageBox.TYPE_INFO, enable_input=False)
		...
		if config.flashExpander.device.value:
			...
			if exists(usrDirectory):
				if self.active:
					self.session.openWithCallback(deactivateCallback, MessageBox, "%s\n\n%s" % (usrDirectory, _("Do you want to deactivate this flash expander?")), MessageBox.TYPE_YESNO, default=False)
				else:
					self.session.openWithCallback(deleteCallback, MessageBox, "%s\n\n%s" % (usrDirectory, _("Do you want to delete this flash expander directory?")), MessageBox.TYPE_YESNO, default=False)
			else:
				copyFlashdata()
		else:
			self.showDeviceSelection()
